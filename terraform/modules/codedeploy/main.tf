...
# CodeDeploy Application - API v2
resource "aws_codedeploy_app" "api_v2" {
...
# CodeDeploy Deployment Group - API v2
resource "aws_codedeploy_deployment_group" "api_v2" {
  app_name               = aws_codedeploy_app.api_v2.name
  deployment_group_name  = "${var.app_name}-v2-${var.environment}"
  deployment_config_name = "CodeDeployDefault.ECSAllAtOnce"
  service_role_arn       = aws_iam_role.codedeploy.arn
  ...
  ecs_service {
    cluster_name = var.ecs_cluster_name
    service_name = var.ecs_service_names[1]
  }
...
output "codedeploy_app_names" {
  value = [
    ...
    aws_codedeploy_app.api_v2.name
  ]
}
...
output "codedeploy_group_names" {
  value = [
    ...
    aws_codedeploy_deployment_group.api_v2.deployment_group_name
  ]
}
